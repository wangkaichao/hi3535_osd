...
static net_alarm_info_t netalarm_info;
static pthread_mutex_t netalarm_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void *SearchThread(void *param)
{
    ...
    while (1) {
        ...
        case NET_CMD_SEARCH:
        {
            ...
            sprintf(msg_broadcast.version, "%s", systeminfo.version_date);
...
#define SAVE_FILE   (0)
  ...
  switch (pmsg->m_head.m_type) {
    ...
    case NET_CMD_ALARM:
    {
      ...
      net_alarm_info_t net_alarm;
      memmove(&net_alarm.m_num, pmsg->m_data, sizeof(int));
      memmove(net_alarm.m_ch, pmsg->m_data + sizeof(int), net_alarm.m_num);
      NetLib_SetNetAlarmInfo(&net_alarm);
      
      printf("[%d]", net_alarm.m_num);
      int i;
      for (i = 0; i < net_alarm.m_num; i++)
        printf("%d", net_alarm.m_ch[i]);
      printf("\n");
...
int NetLib_GetNetAlarmInfo(net_alarm_info_t *pnet_alarm)
{
  if (pnet_alarm == NULL) {
    logErr("param is null");
    return -1;
  }

  Pthread_mutex_lock(&netalarm_mutex);
  *pnet_alarm = netalarm_info;
  Pthread_mutex_unlock(&netalarm_mutex);
  return 0;
}

int NetLib_SetNetAlarmInfo(const net_alarm_info_t *pnet_alarm)
{
  if (pnet_alarm == NULL) {
    logErr("param is null");
    return -1;
...
{
    Pthread_mutex_lock(&netalarm_mutex);
    memset(&netalarm_info, 0, sizeof(net_alarm_info_t));
    Pthread_mutex_unlock(&netalarm_mutex);
